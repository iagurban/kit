 ...
 *     ].sort(
 *       sorter(
 *         sorter.descending(sorter.field(o => o.q)),
 *         sorter.field(o => o.b, {nulls:`first`}) // nulls first (default is nulls-last-mode)
 *       )
 ...
 */
export const sorter = Object.assign(
  <T>(...inSorters: readonly (CompareFunction<T> | undefined)[]): CompareFunction<T> => {
    const sorters = inSorters.filter(isDefined);
    return (a, b) => {
  ...
  },
  {
    field:
      <T, K extends keyof T>(
        getter: (o: T) => T[K],
        compareFnOrOptions?: CompareFunction<T[K]> | { direction?: `asc` | `desc`; nulls?: `first` | `last` }
      ): CompareFunction<T> =>
      (a, b) => {
        const va = getter(a);
        const vb = getter(b);
    ...
    reverse: <T>(fn: CompareFunction<T>, disable?: boolean): CompareFunction<T> =>
