...
export class Requester<T> {
  ...
  @computed
  get wait() {
    return this.requestState?.loading?.promise;
  }
}
...
export class CancelledError<T> extends Error {
  constructor(
    ...
    super(`cancelled request completed with ${result ? `result` : `error`}`);
    console.log(error);
  }
}
...
export class ImperativeRequester<T> {
  ...
  @observable.ref
  private requesting: { promise: Promise<T>; cancel?: () => void; key: unknown } | null = null;
  ...
  @action
  private setResults(result: T | null, error: unknown | null) {
    this.result = result;
    this.error = error;
  }
    ...
    this.requesting = {
      promise: Promise.resolve()
        ...
        .then(result => {
          ...
          // throw new CancelledError(result, null);
          return result;
        })
        .catch(e => {
          ...
          if (key === this.requesting?.key) {
            this.setResults(null, e);
            throw e;
          }
          throw new CancelledError(null, e);
        })
        .finally(() => {
          if (key === this.requesting?.key) {
            runInAction(() => {
  ...
  @computed
  get wait() {
    return this.requesting?.promise;
  }
