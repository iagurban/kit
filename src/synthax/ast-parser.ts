...
import { $o, $t, $u, briefInfo } from './define';
  ...
  unaryOps: () =>
    [
      $o.ops(
        ...
        $o.unary(`~`),
        $o.unary(`!`),
        $o.op(`typeof x`, 'typeof'),
        $o.op(`void x`, 'void'),
        $o.op(`delete x`, 'delete'),
        $o.op(`await x`, 'await')
      ),
    ] as const,
  ...
  assignmentOps: () =>
    [
      [
        ...
        ...$o.ops($o.rtl(1), $o.unary('...'), $o.op(`yield x`, 'yield'), $o.op(`yield* x`, 'yield*')),
      ],
    ] as const,
...
export const definePinqLisplLang = () => {
  ...
  const numberScalar = jsonPresets.positiveNumberMatcher(
    (none, info) => ({ kind: `number`, data: $u.nodeText(info), ...briefInfo(info) }) as const
  );
