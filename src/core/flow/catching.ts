 ...
 * @param {Function} fn - The function to execute, which may throw an error.
 * @param {Function} onCatch - The error-handling function to execute if an error is thrown. Receives the error as its argument.
 * @return {T} The result of the executed function if no error occurs, or the result of the error-handling function if an error is caught.
 */
 ...
 * @template T
 * @param {() => Promise<T>} fn - An asynchronous function that will be executed.
 * @param {(e: unknown) => T | Promise<T>} onCatch - A function that gets invoked with the caught error if `fn` throws or rejects. This function can return a value or a Promise.
 * @return {Promise<T>} A Promise that resolves to the result of `fn` if it succeeds, or to the outcome of `onCatch` if an error is caught.
 */
